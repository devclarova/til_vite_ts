...
- 사용자가 내용 작성 중 이미지를 배치한다면
  ...
  - 3. 이미지를 배치하면 미리보기 URL을 생성한 후 보여주고
    ...
    - 3.1. 사용자가 저장 버튼 누르면 그 때 storage에 등록자 폴더 생성 후 저장하고
    - 3.2. 저장이 성공하면 getURL로 주소 알아내고
    - 3.3. content의 내용 중 img src="주소" 교체하고
    - 3.4. DB에 저장한다.

## 1. Supabase Storage 설정

### 1.1. `todo-images`를 생성
...
### 1.3. 업로드 시 `todo-image/사용자ID/파일들...`
