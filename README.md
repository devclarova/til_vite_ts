...
- 익숙하다면 `sql Editor로 관리 권장 (ChatGPT 활용 권장)
...
## 5. Supabase의 테이블의 칼럼의 `데이터 타입 정의`
...
- `id`는 URL의 앞쪽 단어가 ID가 됨
  - id는 Supabase url의 앞쪽 단어가 ID가 됨
  - 메뉴로 진입 시 `Project Settings` > `General settings`
- package.json 추가

```json
"generate-types": "npx supabase gen types typescript --project-id oilnazmguqucxkbcmonm --schema 경로 > 파일명.ts"
```
...
```json
  "scripts": {
    ...
    "generate-types": "npx supabase gen types typescript --project-id oilnazmguqucxkbcmonm --schema public > types_db.ts"
  },
```

- 생성된 ts의 내용을 참조해서 우리가 원하는 곳에 복사 및 붙여넣기를 권장
