...
- 기본 제공되는 탈퇴 기능
  - `Supabas.auth.admin.deleteUser()`
    - 관리자 전용 (서버에서만 실행됨)
    - react는 클라이언트 즉, 웹브라우저 전용이라서 실행 불가
    - 보안상 위험: 실수로 지울 가능성
    - 복구 불가
- 탈퇴 기능
  - 사용자 비활성
...
CREATE TABLE account_deletion_requests (
  -- pk이고 DEFAULT gen_random_uuid() 중복 안되는 ID 생성
...
function ProfilePage() {
  ...
  const saveProfile = async () => {
    ...
    try {
      const tempUdateData: ProfileUpdate = { nickname: nickName };
      const success = await updateProfile(tempUdateData, user.id);
      if (!success) {
