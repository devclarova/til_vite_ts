...
- int2 (Small Integer): 작은 숫자 (-32768 ~ 32767)
...
- numeric (Decimal): 엄청 정밀한 소숫점 값 (예: 3.1415926535)

## 2. String 종류

- text: 가장 많이 사용하는 타입, 길이 제한이 없어서 html 등 저장
...
## 4. JSON 종류 (구조화된 `{이름: 값, 키명: 값...}`)

- json: 글자임. (예: `{이름: 로제, 나이: 18, ...}`)
- jsonb: 더 빠르게 검색이 가능한 형태의 바이너리 코드로 변환한 자료형.
- 참고사항으로 Firebase, Mongo DB에서 이 타입을 사용함.

## 5. 독특한 데이터 타입

- uuid: 고유한 ID를 저장할 때 (`pk`로 추천)
...
## 6.1. 왜 timestamptz를 권장하는가?

- 세계 어느 나라에 가도 정확한 시간 유지
- 시간대가 바뀌어도 문제 없음
- FE에서 별도로 변환하지 않아도 됨

### 6.2. 회원정보 테이블 (profiles 예제)
...
- `행 단위로 접근을 제한함`
- 다른 DBMS는 RLS가 없음 (DB 관리자가 별도로 작업)
- 데이터를 관리할 때 자격을 제한함
- RLS를 설정하지 않으면 CRUD가 자유로움
- 처음에는 RLS에 대해서 적용하지 않고 개발을 시작함
- 하지만 결국 RLS가 필요하므로 미리 셋팅에 대해서 고민을 해야함
...
on memos
for select
using (is_public = ture OR auth.uid() = user_id);
```
...
- policy도 별도의 테이블에 관리되고 있다.

```sql
select relrowsecurity, relforce
from pg_class
where relname = 'memos';
```
