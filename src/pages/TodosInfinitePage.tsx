...
const InfiniteTodoList = () => {
  ...
  // 4. 데이터 로드 스크롤바 하단에 위치문제로 연속 호출되는 부분 제어
   ...
   * 아직 더 불러올 데이터가 있으면 loadMore를 실행 ==> 데이터 추가
   * 새로운 목록이 랜더링 되면 새로운 마지막 요소에 다시 옵저버를 붙임
   * 위의 과정을 반복 ==> 끝까지 반복
   */

  // 마지막 todo 항목이 화면에 보이면 자동으로 다음 데이터를 불러들이는 함수
  // 여기서는 useCallback을 사용함
  // - 함수가 리랜더링될 때마다 새롭게 만들면 성능 이슈가 있음
  // - 함수가 새로 만들어져야 하는 경우는 의존성 배열에 추가함
        ...
        <div>
          <ul>
            {todos.map((item, index) => (
              // 마지막 요소 태그인지 연결한다. (마지막 배열의 index인지 비교하면 됨)
